...
public class Driver {
    ...
    public static void handleGuessing(Scanner scanner, Player player, Puzzle puzzle, GuessedLetters guessedLetters) {
	    ...
	    public static void playGame(Scanner scanner) {
	        ...
	        PlayerQueue playerQueue = new PlayerQueue();
	        Wheel wheel = new Wheel();
	        PuzzleLinkedList puzzleList = new PuzzleLinkedList();
	        ...
	        // Add puzzles to the game
	        puzzleList.addPuzzleNode(new PuzzleNode(new Puzzle("Movies", "The Godfather")));
	        puzzleList.addPuzzleNode(new PuzzleNode(new Puzzle("Movies", "The Shawshank Redemption")));
	        puzzleList.addPuzzleNode(new PuzzleNode(new Puzzle("Cities", "New York")));
	        puzzleList.addPuzzleNode(new PuzzleNode(new Puzzle("Cities", "Paris")));
	        puzzleList.addPuzzleNode(new PuzzleNode(new Puzzle("Fruits", "Apple")));
	        puzzleList.addPuzzleNode(new PuzzleNode(new Puzzle("Fruits", "Banana")));

	        // Display puzzles to the players
	        puzzleList.displayMaskedPuzzles();
	        ...
	        for (int round = 1; round <= 3; round++) {
	            ...
	            try {
	            	// Game play loop for each player's turn
	            	for (Player player : players) {
	            	    System.out.println("\n" + player.getName() + "'s turn to spin the wheel.");
	            	    ...
	            	    switch (spunCard.getType()) {
	            	        case MONEY:
	            	            ...
	            	            break;
	            	        case LOSE_A_TURN:
	            	            // For simplicity, just outputting a message. Player loses next turn.
	            	            System.out.println(player.getName() + " loses a turn.");
	            	            break;
	            	        case BANKRUPTCY:
	            	            player.setRoundTotal(0); // Reset round total to 0 for bankruptcy
	            	            ...
	            	            break;
	            	        case FREE_PLAY:
	            	            // Player gets the card's value without any additional action
	            	            player.addRoundTotal(spunCard.getValue());
	            	            System.out.println(player.getName() + " gets a free play and wins $" + spunCard.getValue());
	            	            break;
	            	    }
	            	    ...
	            	    // For demonstration, let's print the player's round total after their turn
	            	    System.out.println(player.getName() + "'s Round Total: $" + player.getRoundTotal());

	            	    // Handle guessing letters, solving the puzzle, and buying vowels
	            	    char choice = getUserChoice(); // Assume you have a method to get user input for choice
	            	    switch (choice) {
	            	        case '1':
	            	            // Guess a letter
	            	        ...
	            	        case '2':
	            	            // Solve the puzzle
	            	            String guess = getSolvedPuzzle(); // Assume you have a method to get user input for guess
	            	            player.solvePuzzle(guess, puzzle);
	            	            break;
	            	        case '3':
	            	            // Buy a vowel
	            	            char vowel = getVowel(); // Assume you have a method to get user input for vowel
	            	            player.buyVowel(vowel, puzzle);
	            	            break;
	            	        default:
	                ...
	                /*for (Player player : players) {
	                    ...
	                    switch (spunCard.getType()) {
	                        ...
	                        case LOSE_A_TURN:
	                            ...
	                            break;
	                        case BANKRUPTCY:
	                            player.setGrandTotal(0); // Reset grand total to 0 for bankruptcy
	                            System.out.println(player.getName() + " goes bankrupt.");
	                            break;
	                        case FREE_PLAY:
	                            // Player gets the card's value without any additional action
